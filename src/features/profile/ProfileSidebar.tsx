    ...
    <aside className="relative w-full lg:w-72 lg:shrink-0 space-y-5 rounded-2xl border border-border/60 bg-(--bg-secondary) dark:bg-(--dk-surface) p-4 sm:p-5">
      ...
      {(() => {
        ...
        if (user.githubUsername && user.githubUsername.trim() !== "") {
          activeSocials.push({
            icon: Github,
            href: `https://github.com/${user.githubUsername.replace(/^@/, "")}`,
            label: "GitHub",
        ...
        if (user.linkedinUsername && user.linkedinUsername.trim() !== "") {
          activeSocials.push({
            icon: Linkedin,
            href: `https://linkedin.com/in/${user.linkedinUsername.replace(/^@/, "")}`,
            label: "LinkedIn",
        ...
        if (user.twitterUsername && user.twitterUsername.trim() !== "") {
          activeSocials.push({
            icon: Twitter,
            href: `https://twitter.com/${user.twitterUsername.replace(/^@/, "")}`,
            label: "Twitter",
        ...
        if (user.websiteUrl && user.websiteUrl.trim() !== "") {
          ...
          activeSocials.push({
            icon: Globe,
            href: cleanWebsite.startsWith("http")
              ? cleanWebsite
              : `https://${cleanWebsite}`,
            label: "Website",
            username: cleanWebsite,
          });
        }
      ...
      <div className="space-y-2.5 text-sm px-1">
        ...
        <div className="flex items-center gap-2.5 text-(--text-secondary) dark:text-(--dk-text-muted)">
          ...
          <span className="text-xs">
            Joined {formatDate(user.createdAt, user.memberSince)}
          </span>
        </div>
      </div>
      ...
      {/* Activity Stats instead of Community */}
      <div className="px-1">
      ...
      {/* Skills & Languages */}
      <ProfileSkills languages={stats?.languages || []} />
