...
const getTopicIcon = (name: string) => {
  ...
  if (lowerName.includes("input"))
    return <ChevronsLeftRightEllipsis className="size-[18px]" />;
  if (lowerName.includes("operator"))
    return <Ampersand className="size-[18px]" />;
  if (lowerName.includes("condition"))
    return <ChartNoAxesGantt className="size-[18px]" />;
  if (lowerName.includes("loops")) return <Repeat className="size-[18px]" />;
  if (lowerName.includes("pattern"))
    return <Asterisk className="size-[18px]" />;
  if (lowerName.includes("function"))
    return <ParenthesesIcon className="size-[18px]" />;
  if (lowerName.includes("math") || lowerName.includes("number"))
    return <Hash className="size-[18px]" />;
  if (lowerName.includes("array")) return <Brackets className="size-[18px]" />;
  if (lowerName.includes("string"))
    return <TextInitial className="size-[18px]" />;
...
export default function TopicSidebar({
  ...
  return (
    <div className="h-[calc(100vh-12rem)] sticky top-0 flex flex-col font-pj overflow-hidden">
      {/* My Lists Section */}
      ...
      <ScrollArea className="flex-1 min-h-0 -mx-2">
        <div className=" px-2 pb-6">
          {[...topics]
            ...
            .map((topic, index) => {
              const isActive = selectedTopicId === String(topic.id);
              return (
                <button
                  key={topic.id || index}
                ...
                >
                  ...
                  <Badge
                    ...
                    className={cn(
                      ...
                      isActive
                        ? "bg-(--btn-primary-bg)/20 text-(--btn-primary-bg)"
