...
const EliteTitle: React.FC<{ className?: string }> = ({ className }) => (
  <div className={`relative inline-flex ${className ?? ""}`}>
    {/* Animated border */}
    <div
      className="absolute inset-[-3px] rounded-xl blur-xs"
      style={{
        background:
    ...
    <div className="relative z-10 inline-flex items-center gap-2.5 px-4 py-2 rounded-xl bg-linear-to-r from-slate-900 to-slate-800">
      <Layers className="w-4 h-4 text-amber-400 shrink-0 animate-pulse" />
      <span className="text-sm font-black tracking-wide bg-linear-to-r from-amber-300 to-orange-400 bg-clip-text text-transparent whitespace-nowrap">
        ByteBox Elite
      </span>
    </div>
  </div>
...
const RenewalTitle: React.FC<{ className?: string }> = ({ className }) => (
  <div
    className={`inline-flex items-center gap-2.5 px-4 py-2 rounded-xl bg-linear-to-r from-amber-500/15 to-orange-500/10 border border-amber-500/30 ${className ?? ""}`}
  >
    <RefreshCw className="w-4 h-4 text-amber-400 shrink-0" />
    <span className="text-sm font-black tracking-wide bg-linear-to-r from-amber-400 to-orange-400 bg-clip-text text-transparent whitespace-nowrap">
